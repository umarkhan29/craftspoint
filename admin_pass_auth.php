...
A verification code has been send to <span style="color:#602D8D;"><a href=""><?php echo $_SESSION['admin_forgot_email']; ?></a></span><br>Please Enter that code to continue.<br>
</div>
...
<?php
	...
	if($auth_code==$_SESSION['admin_forgot_pass_auth']){
		...
		if(mysql_query("UPDATE `admins` SET `passcode` ='$new_pass' WHERE `email`='$email' ")){
						echo '<div style="font-family:Arial,Helvetica,sans-serif;width:22%;height:100px;margin-left:25%;font-size:18px; color:green;">Password Changed Sucessfully<br><a href="admin_login.php">Login</a></div>';
						session_destroy();
			}
			else{
