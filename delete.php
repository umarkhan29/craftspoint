...
ob_start();
	...
	if(mysql_query("DELETE FROM `gallery` WHERE `id` = $ITEM_ID")){
			unlink($ITEM_PATH);
			header('location:gallery_delete.php?deletemsg=Sucessfully Deleted');
	}
	else{
		header('location:gallery_delete.php?deletemsg=Something Went Wrong');
			}
			ob_end_flush();
