<?php
	...
	$catagory="carpets";
	$material="wool";
		$products=mysql_query("SELECT * FROM `products` WHERE `catagory` = '$catagory' and `material` like '%$material%' ") or die(header('location:error.php?token=dbt'));
		if(isset($_POST['sort_btn'])){
		if($_POST['sort']=="Lowest Price"){
			$products=mysql_query("SELECT * FROM `products` WHERE `catagory` = '$catagory' and `material` like '%$material%' ORDER BY `discount` ASC ") or die(header('location:error.php?token=dbt'));
		}elseif($_POST['sort']=="Highest Price"){
			$products=mysql_query("SELECT * FROM `products` WHERE `catagory` = '$catagory' and `material` like '%$material%' ORDER BY `discount` DESC ") or die(header('location:error.php?token=dbt'));
		}
	}
		$count=0;
		if($products){
			...
			?>
				<div class="grid_1_of_4 images_1_of_4">
					 ...
					 <p>Rs <span class="strike"><?php echo $items[$i]['PRICE']?></span><span class="price"><?php echo $items[$i]['DISCOUNT'];?></span></p>
					  <div class="button"><span><img src="images/cart.jpg" alt="" /><a href="home/catalog/add_to_cart.php?product_id=<?php echo $items[$i]['ID'];?>" class="cart-button">Add to Cart</a></span> </div>
