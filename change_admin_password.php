		...
		<div style="margin-top:-40px">
		  ...
		  <table>
							...
							if($_POST['oldpass']==""){
								...
								echo "Enter UserName";
									...
									if(preg_match("%^[a-z A-Z 0-9 .,!@&$]{4,26}$%",$_POST['oldpass'])){
										if($_SESSION['admin_password']!= md5(md5(hash('sha512',md5(base64_encode(hash('sha1',$_POST['oldpass']))))))){
           ...
           <div class="form_row">
			...
			<div class="form_row" style="font-size:14px; color:#666666;">
            Go back to <a href="admin_section.php" >Admin Home</a>
            </div>
          </form>
...
<?php
	if(isset($_POST['loginbtn'])){
		if($admin=="1"){
			...
			$query="UPDATE `admins` SET `passcode` = '$user_pass' WHERE `username` = '$user_name';";
			
			if(mysql_query($query)){
			
					$_SESSION['admin_password']=$user_pass;
